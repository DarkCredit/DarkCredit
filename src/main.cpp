...
unsigned int nTargetSpacing = 1 * 65; // 1 minute and 5 second blocks
unsigned int nStakeMinAge = 8 * 60 * 60; // 8 hours
unsigned int nStakeMaxAge = -1;           //unlimited
unsigned int nModifierInterval = 10 * 60; // time to elapse before new modifier is computed

int nCoinbaseMaturity = 200; //200 blocks is good
...
const string strMessageMagic = "DarkCredit Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    {
		nSubsidy = 0 * COIN; // 99 empty blocks
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "DarkCredit", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Obama won't rule out sending weapons to Ukraine 2/9/15";
        CTransaction txNew;
        txNew.nTime = 1423515363;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1423515363;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 7366235;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x807ce0bf2610dd572d637ac29bbcb6fdcbfea4d5e61fe3dfff132060cbf9c9a2"));
        //block.print();
...
bool static Procesdbmessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
...
bool Procesdbmessages(CNode* pfrom)
{
    //if (fDebug)
    //    printf("Procesdbmessages(%zu messages)\n", pfrom->vRecvMsg.size());
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        //if (fDebug)
        //    printf("Procesdbmessages(message %u msgsz, %zu bytes, complete:%s)\n",
        ...
        if (memcmp(msg.hdr.pchMessageStart, pchMessageStart, sizeof(pchMessageStart)) != 0) {
            printf("\n\nPROCESdbmessage: INVALID MESSAGESTART\n\n");
            fOk = false;
            break;
        }
        ...
        if (!hdr.IsValid())
        {
            printf("\n\nPROCESdbmessage: ERRORS IN HEADER %s\n\n\n", hdr.GetCommand().c_str());
            continue;
        }
        ...
        {
            printf("Procesdbmessages(%s, %u bytes) : CHECKSUM ERROR nChecksum=%08x hdr.nChecksum=%08x\n",
        ...
        try
        {
            {
                LOCK(cs_main);
                fRet = Procesdbmessage(pfrom, strCommand, vRecv, msg.nTime);
            }
            if (fShutdown)
                break;
        }
        catch (std::ios_base::failure& e)
        {
            if (strstr(e.what(), "end of data"))
            {
                ...
                printf("Procesdbmessages(%s, %u bytes) : Exception '%s' caught, normally caused by a message being shorter than its stated length\n", strCommand.c_str(), nMessageSize, e.what());
            }
            else if (strstr(e.what(), "size too large"))
            {
                // Allow exceptions from over-long size
                printf("Procesdbmessages(%s, %u bytes) : Exception '%s' caught\n", strCommand.c_str(), nMessageSize, e.what());
            }
            else
            {
                PrintExceptionContinue(&e, "Procesdbmessages()");
            }
        }
        catch (std::exception& e) {
            PrintExceptionContinue(&e, "Procesdbmessages()");
        } catch (...) {
            PrintExceptionContinue(NULL, "Procesdbmessages()");
        }

        if (!fRet)
            printf("Procesdbmessage(%s, %u bytes) FAILED\n", strCommand.c_str(), nMessageSize);
    }
